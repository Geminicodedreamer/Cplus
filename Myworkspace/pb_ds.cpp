...
// Node_Update：用于更新节点的策略，默认使用 null_node_update，若要使用 order_of_key 和 find_by_order 方法，需要使用 tree_order_statistics_node_update(该方法是在统计子树的s i z e sizesize)
// Allocator：空间分配器类型
// 成员函数：
...
// tr.join(b);   // 将b并入tr
// pair          // pair的使用如下：
//     pair<tr::iterator, tr::iterator>
//         range = base.prefix_range(x);
...
// 其余方法同std::map，但是注意，该数据结构的总复杂度是O ( N ) O(N)O(N)。
...
// (5).可持久化数组 / 可持久化平衡树 / 块状链表 rope 要求引入头文件
// #include <ext/rope>
...
//     rp;
// 1 2 成员函数
// push_back(x) : 在末尾插入x xx

// insert(pos, x) : 在p o s pospos处插入x xx

// erase(pos, x) : 在p o s pospos处删除x xx个元素

// length() : 返回数组长度

// size() : 返回数组长度(同上)

// replace(pos, x) : 将p o s pospos处元素替换为x xx

// substr(pos, x, s) : 从p o s pospos处开始提取x xx个元素

// copy(pos, x, s) : 从p o s pospos处开始复制x xx个元素到s ss中

// at(x) : 访问第x xx个元素，同rp[x]
